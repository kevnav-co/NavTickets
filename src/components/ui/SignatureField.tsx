  ...
  return (
    <div className="space-y-3">
      ...
      {!savedSignature ? (
        ...
        <div onClick={() => !disabled && setIsModalOpen(true)} className={`relative h-32 bg-white border border-gray-200 rounded-[1.8rem] overflow-hidden shadow-inner transition-all ${!disabled ? 'cursor-pointer hover:border-[#7b1113]' : ''}`}>
          <img src={savedSignature} className="w-full h-full object-contain p-2" alt="Firma" />
          {!disabled && (<div className="absolute inset-0 bg-black/0 hover:bg-black/5 flex items-center justify-center opacity-0 hover:opacity-100 transition-all"> <div className="bg-white/90 backdrop-blur-sm p-2 rounded-xl shadow-md text-[#7b1113]"><Maximize2 size={16} /></div> </div>)}
        </div>
      )}
