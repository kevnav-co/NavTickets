...
const GlobalCalendar: React.FC = () => {
  ...
  const newOrderState = useMemo(() => {
    ...
    return { date: slotStartDate.toISOString(), duration, technicianId };
  ...
  // Build QueryFilters for pending and open orders
  ...
  const { data: pendingOrders, loading: loadingPending } = useCollection<ServiceOrder>('orders', { filters: pendingConstraints });
  const { data: openOrders, loading: loadingOpen } = useCollection<ServiceOrder>('orders', { filters: openConstraints });
  ...
  const weeklyLayout = useMemo(() => {
    const weekData = weekDays.map(day => {
      ...
      const processGroup = (group: any[]) => {
        ...
        return columns.map(col => col.map(order => ({ ...order, colIndex: col.indexOf(order), totalCols: columns.length }))).flat();
      };

      const allEventsWithLayout: any[] = [];
      let dayMaxCols = 1;
    ...
    const totalFr = weekData.reduce((sum, d) => sum + d.dayMaxCols, 0);
                  ...
                  return (
                    <div key={dayIndex} className={`border-r border-gray-200 last:border-0 relative h-full flex flex-col ${isGraySlot ? 'bg-gray-200/40' : 'bg-white'}`}>
                      {[0, 15, 30, 45].map(m => {
                        ...
                        return (
                          <div key={m} className={`flex-1 border-b border-dashed border-gray-100 last:border-b-0 ${canClick ? 'hover:bg-red-500/10 cursor-pointer' : ''}`} onClick={() => canClick && setDraftSlot({ date: weekDays[dayIndex], hour: h, minute: m, dayIndex, durationMinutes: 60 })} />
                        );
                      })}
                    </div>
                  );
            ...
            {draftSlot && newOrderState && (
              <div className="absolute bg-primary/10 border-l-[4px] border-primary rounded-r-lg shadow-xl z-30 flex flex-col justify-center box-border px-1 select-none"
                ...
                <div className="absolute right-1 top-1/2 -translate-y-1/2">
                  <Link to="/orders/new" state={newOrderState} className="bg-primary text-white w-6 h-6 rounded-full flex items-center justify-center shadow-md hover:scale-110 transition-transform">
                    <Plus size={14} strokeWidth={3} />
                  </Link>
                </div>
                <div className="w-full h-3 absolute bottom-0 cursor-ns-resize flex items-end justify-center pb-0.5" onMouseDown={e => handleDragStart(e, 'resize-bottom')} onTouchStart={e => handleDragStart(e, 'resize-bottom')}>
            ...
            {weeklyLayout.weekData.map((dayLayout, dayIndex) => (
              <div key={dayIndex} className="absolute top-0 h-full pointer-events-none" style={{ left: `calc(60px + (100% - 60px) * ${weeklyLayout.weekData.slice(0, dayIndex).reduce((p, c) => p + c.dayMaxCols, 0)} / ${weeklyLayout.totalFr})`, width: `calc((100% - 60px) * ${dayLayout.dayMaxCols} / ${weeklyLayout.totalFr})` }}>
                {dayLayout.events.map(o => {
                  ...
                  const displayTime = o.status === OrderStatus.OPEN && o.startTime ? new Date(o.startTime).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' }) : o.timeSlot;
                  return (
